    ...
    <div class="card-body">
        <form method="post" action="{{route('product.update',$product->id)}}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <small class="form-text text-muted d-block mt-2">Upload new image to replace current photo (JPG, PNG, GIF)</small>
                @if($product->photo)
                <small class="d-block mt-2">Current photo: {{basename($product->photo)}}</small>
                @endif
                @error('photo')
                <span class="text-danger">{{$message}}</span>
                @enderror
